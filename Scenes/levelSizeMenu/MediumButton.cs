...
public partial class MediumButton : TextureButton
{
		/// <summary>
	/// The button alters the global "levelSize" variable when pushed. This button sets
	/// the size to 10. The button then broadcasts a signal which resets the state of all other
	...
	public override void _Ready()
	{
		...
		if (settings.getLevelSize() == 10) // disables this button if the level size is already 10.
		{
			this.Disabled = true;
		}
	}
	...
	void onPressed()
	{
		settings.setLevelSize(10); // calls "levelSize"'s setter function with parameter '10'
