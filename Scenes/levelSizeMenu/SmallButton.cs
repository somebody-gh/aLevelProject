...
public partial class SmallButton : TextureButton
{
		/// <summary>
	/// The button alters the global "levelSize" variable when pushed. This button sets
	/// the size to 5. The button then broadcasts a signal which resets the state of all other
	...
	public override void _Ready()
	{
		...
		if (settings.getLevelSize() == 5) // disables this button if the level size is already 5.
		{
			this.Disabled = true;
		}
	}
	...
	void onPressed()
	{
		settings.setLevelSize(5); // calls "levelSize"'s setter function with parameter '5'
